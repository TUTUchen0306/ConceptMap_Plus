...
#from download_youtube_code.api import youtube_search
#from download_youtube_code.api import video_get_info
#from download_youtube_code.api import video_get_channel_info
#from download_youtube_code.api import get_comment_threads

#from download_youtube_code.auth_api import 
import download
import os
import json
import pickle
...
def index():
    return send_file(ROOT + 'NLP_test.json')
...
def GetJson():
    f = open('vis_json/NLP_test.json','r')
    data = json.load(f)
    resp = Response(json.dumps(data))
    resp.headers['Access-Control-Allow-Origin'] = '*'
    resp.headers['Content-Type'] = 'application/json'
